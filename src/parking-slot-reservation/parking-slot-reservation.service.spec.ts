...
describe('ParkingSlotReservationService', () => {
  ...
  const parkingSlotReservationEntityList: Partial<ParkingSlotReservation>[] = [
    { slotId: 1, startTimestamp: addMilliseconds(new Date(), 1), exitTimestamp: addMilliseconds(new Date(), 1), durationInMinutes: 10, bookingDate: addMilliseconds(new Date(), 1), numberPlate: 'กท.1111', carSize: ECarSize.SMAILL },
    { slotId: 1, startTimestamp: addMilliseconds(new Date(), 2), exitTimestamp: addMilliseconds(new Date(), 2), durationInMinutes: 10, bookingDate: addMilliseconds(new Date(), 2), numberPlate: 'กท.2222', carSize: ECarSize.SMAILL },
    { slotId: 1, startTimestamp: addMilliseconds(new Date(), 3), exitTimestamp: addMilliseconds(new Date(), 3), durationInMinutes: 10, bookingDate: addMilliseconds(new Date(), 3), numberPlate: 'กท.3333', carSize: ECarSize.SMAILL }
  ];
  ...
  beforeEach(async () => {
      ...
      providers: [
        ...
        SlotsService,
        {
          ...
          useValue: {
            ...
            findAll: jest.fn().mockResolvedValue(parkingSlotReservationEntityList),
    ...
    parkingSlotReservationService = module.get<ParkingSlotReservationService>(ParkingSlotReservationService);
    parkingSlotReservationRepository = module.get<Repository<ParkingSlotReservation>>(getRepositoryToken(ParkingSlotReservation));
  });
